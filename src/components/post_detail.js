      ...
      <div className="col-md-8 col-md-offset-2">
       <div className="read-detail">
         <div className="edit-post">
           <span className="glyphicon glyphicon-pencil"><Link to={`/post/${post.id}/edit`}>edit</Link></span>
         </div>
         ...
         <p className="lead"><span className="glyphicon glyphicon-user margin-right:10px"> {post.author}</span></p>

         <ul className="li-bundle">

           <li><span className="glyphicon glyphicon-time">
             {new Date(post.timestamp).toLocaleDateString()}</span></li>
           <li><Link to={`/${post.category}/posts`}> {post.category}</Link></li>
           <li><span className="glyphicon glyphicon-comment"> {noOfComments}</span></li>
           <li><span className="glyphicon glyphicon-star-empty"></span> {post.voteScore}</li>
           <li onClick={() => this.props.upVote(post.id)} >
             <span className="glyphicon glyphicon glyphicon-thumbs-up cursor"></span></li>
           <li onClick={() => this.props.downVote(post.id)}>
             <span className="glyphicon 	glyphicon glyphicon-thumbs-down cursor"></span></li>
